				...
				if (typeof message === 'string') {
					...
					switch (data[0]) {
						case 'role:specify':
							...
							ws.subject_id = v4();
							this.dashboard.log(
								`${ws._socket.remoteAddress} joined the network as ${ws.role}`
							);
							break;
						case 'file:stats':
							ws.file = {
						...
						case 'get:chunk':
							...
							distributor.send(
								JSON.stringify([
									'get:chunk',
									{ ...data[1], subject_id: ws.subject_id },
								])
							);
							break;
						case 'chunk':
							this.wss.clients.forEach((e: any) => {
								if (
									e.role === 'client' &&
									e.id === ws.id &&
									e.subject_id === data[1].subject_id
								) {
									e.send(JSON.stringify(['chunk', { chunk: data[1].chunk }]));
									this.dashboard.log(
										`Transferring ${bytes.formatBytes(
						...
						case 'get:file:hash':
							...
							this.wss.clients.forEach((e: any) => {
								if (
									...
									e.send(
										JSON.stringify(['post:file:hash', { hash: dstr.file.hash }])
									);
								}
							});
							break;
						case 'post:file:hash':
							this.wss.clients.forEach((e: any) => {
								if (
									...
									e.send(
										JSON.stringify(['post:file:hash', { hash: data[1].hash }])
									);
								}
							});
							break;
